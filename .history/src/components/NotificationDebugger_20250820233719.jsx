    ...
    <div className="fixed bottom-4 right-4 bg-slate-800 border border-slate-600 rounded-lg shadow-xl p-4 w-80 max-h-96 overflow-y-auto z-50">
      ...
      <div className="space-y-3">
              ...
              <div className="mt-2 p-2 bg-green-900/20 border border-green-500/30 rounded">
                ...
                {debugInfo.paidRestaurantsDetails.map((restaurant, index) => (
                  <div key={index} className="text-xs text-green-300 mb-1">
                    ...
                    <div>
                      Precio: ${restaurant.precio} {restaurant.moneda}
                    </div>
        ...
        <div className="space-y-2">
          ...
          <button
            onClick={async () => {
              // Simular actualización de un restaurante para probar el listener
              if (db && debugInfo.paidRestaurantsDetails.length > 0) {
                const testRestaurant = debugInfo.paidRestaurantsDetails[0];
                ...
                try {
                  ...
                  console.log("✅ Simulación de actualización enviada");
                } catch (error) {
                  console.error("❌ Error en simulación:", error);
                }
              }
            }}
            className="w-full bg-yellow-600 text-white px-2 py-2 rounded text-xs font-medium hover:bg-yellow-700 transition-colors"
          >
