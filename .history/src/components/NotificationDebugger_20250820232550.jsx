...
import { Bug, Wifi, WifiOff, AlertCircle, CheckCircle, DollarSign, RefreshCw } from "lucide-react";
    ...
    <div className="fixed bottom-4 right-4 bg-slate-800 border border-slate-600 rounded-lg shadow-xl p-4 w-80 max-h-96 overflow-y-auto z-50">
      ...
      <div className="space-y-3">
        <div className="bg-slate-700 rounded p-3">
          <h4 className="text-xs font-semibold text-slate-300 mb-2">Estado del Sistema</h4>
          ...
          <div className="space-y-2 text-xs">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-1">
                ...
                <span className={debugInfo.firebaseStatus === "connected" ? "text-green-400" : debugInfo.firebaseStatus === "error" ? "text-red-400" : "text-yellow-400"}>
                  {debugInfo.firebaseStatus === "connected" ? "Conectado" : debugInfo.firebaseStatus === "error" ? "Error" : "Verificando..."}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-white font-mono">{debugInfo.restaurantsCount}</span>
            </div>

            <div className="flex items-center justify-between">
              <span className="text-slate-400">Con pagos aprobados:</span>
              <span className="text-green-400 font-mono">{debugInfo.paidRestaurantsCount}</span>
            </div>
          </div>
        ...
        <div className="bg-slate-700 rounded p-3">
          <h4 className="text-xs font-semibold text-slate-300 mb-2">Notificaciones Actuales</h4>
          
          <div className="space-y-1 text-xs">
            <div className="flex items-center justify-between">
              <span className="text-slate-400">Total:</span>
              <span className="text-white font-mono">{notifications.length}</span>
            </div>
