...
const PaymentModal = ({ 
  ...
  useEffect(() => {
    if (isOpen && restaurantData && amount) {
      generatePaymentLink();
    }
  ...
  const sendWhatsApp = () => {
    if (paymentLink) {
      const message = `Hola! Aquí tienes el link para activar tu restaurante por $${amount} ${restaurantData.moneda || "ARS"}: ${paymentLink}`;
  ...
  const sendEmail = () => {
    if (paymentLink) {
      ...
      const body = `Hola,\n\nAquí tienes el link para activar tu restaurante por $${amount} ${restaurantData.moneda || "ARS"}:\n\n${paymentLink}\n\nSaludos.`;
          ...
          {step === "processing" && (
            <div className="text-center">
              ...
              <p className="text-gray-600">
                ...
                <strong>{restaurantData?.nombre || restaurantData?.name}</strong>
              </p>
              <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                <p className="text-sm text-blue-800">
                  <strong>Monto:</strong> ${amount} {restaurantData?.moneda || "ARS"}
                </p>
              </div>
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <div className="text-sm text-green-800 space-y-1">
                  <p><strong>Restaurante:</strong> {restaurantData?.nombre || restaurantData?.name}</p>
                  <p><strong>Monto:</strong> ${amount} {restaurantData?.moneda || "ARS"}</p>
                  <p><strong>Estado:</strong> Listo para enviar al cliente</p>
                </div>
              </div>
          ...
          {step === "error" && (
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-4">
                {error || "Ocurrió un error inesperado al generar el link de pago."}
              </p>
              <button
