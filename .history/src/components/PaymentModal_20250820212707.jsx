...
const PaymentModal = ({
  ...
  const generatePaymentLink = async () => {
    ...
    if (!restaurantData.codigoActivacion && !restaurantData.nombre) {
      setError(
        "El restaurante debe tener un código de activación y nombre válido"
      );
      setStep("error");
      return;
    }
    ...
    try {
      // Usar código de activación como identificador único
      const restaurantId = restaurantData.codigoActivacion;
      ...
      let errorMessage = err.message || "Error al generar el link de pago";
      
      if (errorMessage.includes("Mercado Pago no está configurado")) {
        errorMessage = "Error de configuración: Mercado Pago no está configurado correctamente. Verifica las credenciales en .env.local";
      } else if (errorMessage.includes("Firebase no está configurado")) {
        errorMessage = "Error de configuración: Firebase no está configurado correctamente. Verifica las credenciales en .env.local";
      }
              ...
              <div className="bg-gray-50 p-3 rounded-lg mb-4 text-left">
                ...
                <div className="text-sm text-gray-600 space-y-1">
                  ...
                  <p>
                    ...
                    {restaurantData?.codigoActivacion || "No disponible"}
                  </p>
                  <p>
