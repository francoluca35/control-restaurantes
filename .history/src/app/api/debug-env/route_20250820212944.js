...
export async function GET() {
  try {
    ...
    const firebaseVars = {
      ...
      storageBucket: process.env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET ? "✅ Configurada" : "❌ No configurada",
      messagingSenderId: process.env.NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID ? "✅ Configurada" : "❌ No configurada",
      appId: process.env.NEXT_PUBLIC_FIREBASE_APP_ID ? "✅ Configurada" : "❌ No configurada",
    };

    // Verificar variables de MercadoPago
    const mercadopagoVars = {
      accessToken: process.env.MERCADOPAGO_ACCESS_TOKEN ? "✅ Configurada" : "❌ No configurada",
      publicKey: process.env.NEXT_PUBLIC_MERCADOPAGO_PUBLIC_KEY ? "✅ Configurada" : "❌ No configurada",
    };

    // Verificar valores reales (sin mostrar las claves completas por seguridad)
    const firebaseValues = {
      apiKey: process.env.NEXT_PUBLIC_FIREBASE_API_KEY ? 
        `${process.env.NEXT_PUBLIC_FIREBASE_API_KEY.substring(0, 10)}...` : "No configurada",
    ...
    const mercadopagoValues = {
      accessToken: process.env.MERCADOPAGO_ACCESS_TOKEN ? 
        `${process.env.MERCADOPAGO_ACCESS_TOKEN.substring(0, 15)}...` : "No configurada",
      publicKey: process.env.NEXT_PUBLIC_MERCADOPAGO_PUBLIC_KEY || "No configurada",
    };
