...
export async function POST(request) {
  try {
    console.log(
      "🔄 Iniciando actualización de transacciones con DNI y nombre completo..."
    );
          ...
          if (restaurantId) {
            try {
              const restaurantDoc = await getDoc(
                doc(db, "restaurantes", restaurantId)
              );
              ...
              console.warn(
                `⚠️ No se pudo obtener datos del restaurante ${restaurantId}:`,
                error
              );
            }
          }
