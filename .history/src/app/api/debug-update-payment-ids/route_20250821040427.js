...
export async function POST(request) {
  try {
    ...
    for (const docSnapshot of paymentsSnapshot.docs) {
      ...
      const hasCorrectFormat = paymentData.paymentId && 
      ...
      if (!hasCorrectFormat) {
        try {
          ...
          if (paymentData.paymentMethod === "efectivo") {
            ...
            const moneda = paymentData.moneda || paymentData.currency || "ARS";
            const mercadopagoId = paymentData.mercadopagoPaymentId || paymentData.paymentId || transactionId;
        ...
        console.log(`⏭️ Transacción ${transactionId} ya tiene paymentId correcto: ${paymentData.paymentId}`);
        skippedCount++;
      }
    }
