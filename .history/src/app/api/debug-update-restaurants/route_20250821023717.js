    ...
    for (const docSnapshot of restaurantsSnapshot.docs) {
      ...
      if (restaurantData.es_usd === undefined) {
        try {
          ...
          for (const payment of payments) {
            if (payment.paymentMethod === "mercadopago") {
              ...
              if (payment.moneda === "USD" || payment.currency === "USD") {
                isUSD = true;
                break;
              } else if (payment.moneda === "ARS" || payment.currency === "ARS") {
                isUSD = false;
                break;
              }
            } else if (payment.paymentMethod === "efectivo") {
              // Para pagos en efectivo, usar el ID del pago
              if (payment.paymentId && payment.paymentId.toUpperCase().includes("USD")) {
                isUSD = true;
                break;
              } else if (payment.paymentId && payment.paymentId.toUpperCase().includes("ARS")) {
                isUSD = false;
                break;
              }
            }
          ...
          console.log(`✅ Restaurante ${restaurantId} actualizado: es_usd = ${isUSD}`);
          updatedCount++;
        } catch (error) {
          console.error(`❌ Error actualizando restaurante ${restaurantId}:`, error);
          errorCount++;
        }
      }
    }

    console.log(`📊 Actualización completada: ${updatedCount} restaurantes actualizados, ${errorCount} errores`);
