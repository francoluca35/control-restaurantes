...
async function recordPaymentTransaction(
  ...
  try {
    ...
    const isAnnual = paymentData.external_reference && 
                    paymentData.external_reference.toLowerCase().includes("anual");
    
    // Generar ID de pago según el formato requerido
    const paymentIdPrefix = currency === "USD" ? "USD" : "ARS";
    const paymentId = `${paymentIdPrefix}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
