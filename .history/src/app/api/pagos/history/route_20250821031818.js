...
export async function GET(request) {
  try {
    ...
    if (!restaurantId) {
      ...
      const totalAmount = payments.filter(p => p.status === "approved").reduce((sum, p) => sum + (p.amount || 0), 0) +
                         transactions.filter(t => t.status === "approved").reduce((sum, t) => sum + (t.amount || t.transaction_amount || 0), 0);
