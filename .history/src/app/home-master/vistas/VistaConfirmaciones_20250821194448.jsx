...
export default function VistaConfirmaciones() {
  ...
  // Función para obtener el tipo de servicio de forma consistente
  const getTipoServicio = (pago) => {
    // Si tipoServicio está definido, usarlo como fuente principal
    if (pago.tipoServicio) {
      return pago.tipoServicio === "conFinanzas"
        ? "Con Finanzas"
        : "Sin Finanzas";
    }

    // Si no hay tipoServicio, usar conFinanzas como fallback
    if (pago.conFinanzas !== undefined) {
      return pago.conFinanzas ? "Con Finanzas" : "Sin Finanzas";
    }
  ...
  // Función para obtener el estado de finanzas de forma consistente
  const getConFinanzas = (pago) => {
    // Si tipoServicio está definido, usarlo como fuente principal
    if (pago.tipoServicio) {
      return pago.tipoServicio === "conFinanzas" ? "Sí" : "No";
    }

    // Si no hay tipoServicio, usar conFinanzas como fallback
    if (pago.conFinanzas !== undefined) {
      return pago.conFinanzas ? "Sí" : "No";
    }
  ...
  const handleDownloadContract = async (pago) => {
    try {
      ...
      console.log("📋 Tipo de servicio del pago:", pago.tipoServicio);
      ...
      const restaurant = {
        ...
        finanzas: getConFinanzas(pago) === "Sí", // Usar la función consistente
        tipoServicio: getTipoServicio(pago), // Usar la función consistente
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-gray-950 to-slate-900 p-6">
              ...
              <div className="flex items-center space-x-4">
                <div className="relative">
                  <input
                    ...
                    className="w-64 px-4 py-2 pl-10 bg-white/20 border border-white/30 rounded-xl text-white placeholder-purple-200 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent backdrop-blur-sm"
                  />
                  <FaEye className="absolute left-3 top-1/2 transform -translate-y-1/2 text-purple-300 w-4 h-4" />
                </div>
                <button
                  onClick={() => setShowFilters(!showFilters)}
                  className="px-4 py-2 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 rounded-xl text-white font-medium transition-all duration-300 transform hover:scale-105 shadow-lg"
                >
