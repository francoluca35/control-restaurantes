...
export default function VistaConfirmaciones() {
  ...
  const updateOldTransactions = async () => {
    try {
      ...
      const response = await fetch("/api/debug-update-old-transactions", {
        method: "POST",
      });
      ...
      if (result.success) {
        console.log("✅ Transacciones actualizadas:", result.message);
  ...
  const handleDownloadContract = async (pago) => {
    try {
      ...
      if (camposFaltantes.length > 0) {
        ...
        console.warn("⚠️ Este pago puede ser de una versión anterior del sistema");
      }
      ...
      const restaurant = {
        ...
        codigoActivacion:
          pago.codigoActivacion || pago.externalReference || pago.restaurantId || pago.id,
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-3">
              ...
              >
                <FaSync className={`w-4 h-4 ${updatingTransactions ? "animate-spin" : ""}`} />
                <span>{updatingTransactions ? "Actualizando..." : "Actualizar Transacciones Antiguas"}</span>
              </button>
              <button
