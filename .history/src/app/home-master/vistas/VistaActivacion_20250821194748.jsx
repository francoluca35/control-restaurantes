...
export default function VistaActivacion() {
  ...
  const { processRestaurantPayment, loading: mercadopagoLoading } =
    useMercadoPago();
  const {
    rates,
  ...
  // Resetear moneda cuando cambia la forma de pago
  React.useEffect(() => {
    if (
      (watchedValues.formaPago === "efectivo" ||
        watchedValues.formaPago === "mercadopago") &&
      !watchedValues.moneda
    ) {
      setValue("moneda", "USD");
    }
  }, [watchedValues.formaPago, watchedValues.moneda, setValue]);
  ...
  const precios = {
    ...
    prueba: { anual: 0.01, mensual: 0.01 },
  };
  ...
  const tipoServicioOptions = [
    {
      value: "sinFinanzas",
      label: "Sin Finanzas - $29.99/mes o $299.99/año ",
    },
    {
      value: "conFinanzas",
      label: "Con Finanzas - $49.99/mes o $499.99/año ",
    },
    {
      value: "prueba",
      label: "Prueba - $10/mes o $10/año ",
    },
  ];

  const formaPagoOptions = [
    { value: "efectivo", label: "Efectivo" },
    { value: "mercadopago", label: "Mercado Pago (Tarjeta de Crédito/Débito)" },
  ];
  ...
  // Calcular precio según selección y moneda
  const precioCalculado = useMemo(() => {
    if (watchedValues.tipoServicio && watchedValues.periodicidad) {
      ...
      // Si la moneda seleccionada es ARS, convertir USD a ARS y aplicar recargo del 15%
      if (watchedValues.moneda === "ARS") {
        const precioARS = convertUsdToArs(precioUSD);
        return precioARS * 1.15; // Recargo del 15% para pagos en pesos argentinos
      }
    ...
    watchedValues.tipoServicio,
    watchedValues.periodicidad,
    watchedValues.moneda,
    convertUsdToArs,
  ]);
  ...
  const generatePDF = useCallback(
    (data) => {
      ...
      doc.text(`Moneda: ${data.moneda || "USD"}`, 20, 170);
      doc.text(
        `Precio: ${formatPrice(precioCalculado, data.moneda || "USD")}`,
        20,
        180
      );
  ...
  const onSubmit = useCallback(
    async (data) => {
      try {
        ...
        // Determinar el estado del pago según la forma de pago
        let estadoPago = "pendiente";
        let fechaPago = null;
        let ingresosMensuales = 0;

        if (data.formaPago === "efectivo") {
          // Si es efectivo, se marca como pagado y se suma a ingresos mensuales
          estadoPago = "pagado";
          fechaPago = new Date();
          ingresosMensuales = precioCalculado;
        }
        ...
        console.log("Validando datos con esquema Zod...");
        try {
          const validation = schemas.restaurant.parse(restaurantData);
        ...
        console.log("🏪 Restaurante creado completo:", createdRestaurant);
        console.log(
          "🔍 Propiedades del restaurante:",
          Object.keys(createdRestaurant)
        );
        console.log("🔍 Tipo de restaurante:", typeof createdRestaurant);

        // Si el pago es con Mercado Pago, mostrar el modal de pago
        if (data.formaPago === "mercadopago") {
          console.log("🏪 Restaurante creado:", createdRestaurant);
          console.log("💰 Precio calculado:", precioCalculado);

          // Asegurar que el restaurante tenga todos los datos necesarios
          const restaurantWithData = {
            ...createdRestaurant,
            nombre: data.nombre, // Asegurar que el nombre esté presente
            codigoActivacion: data.codigoActivacion,
            moneda: data.moneda || "ARS",
          };

          console.log(
            "🏪 Datos del restaurante para el modal:",
            restaurantWithData
          );
          console.log("💰 Monto para el modal:", precioCalculado);

          setCreatedRestaurant(restaurantWithData);
          setShowPaymentModal(true);
          return; // No continuar con el flujo normal
        }

        // Si el pago es en efectivo, registrar el pago directamente
        if (data.formaPago === "efectivo") {
          console.log("💰 Registrando pago en efectivo...");
          setWasCashPayment(true);

          // Buscar el ID del restaurante en diferentes propiedades posibles
          let restaurantId = null;

          // Intentar encontrar el ID real del restaurante
          if (createdRestaurant.id) {
            restaurantId = createdRestaurant.id;
          } else if (createdRestaurant.docId) {
            restaurantId = createdRestaurant.docId;
          } else if (createdRestaurant._id) {
            restaurantId = createdRestaurant._id;
          } else if (createdRestaurant.restaurantId) {
            restaurantId = createdRestaurant.restaurantId;
          } else if (createdRestaurant.ref && createdRestaurant.ref.id) {
            restaurantId = createdRestaurant.ref.id;
          } else {
            // Si no encontramos el ID real, usar el código de activación
            restaurantId = data.codigoActivacion;
            console.warn(
              "⚠️ No se encontró ID real del restaurante, usando código de activación"
            );
          }

          console.log("🏪 Restaurant ID encontrado:", restaurantId);
          console.log("🏪 Restaurant Name:", data.nombre);
          console.log("💰 Amount:", precioCalculado);
          console.log(
            "📝 Description:",
            `Activación de restaurante - ${data.tipoServicio} - ${data.periodicidad}`
          );

          // Registrar el pago en efectivo con todos los datos del formulario
          try {
            const paymentResponse = await fetch("/api/pagos/efectivo", {
              method: "POST",
              headers: {
              ...
              body: JSON.stringify({
                ...
                amount: precioCalculado,
                description: `Activación de restaurante - ${data.tipoServicio} - ${data.periodicidad}`,
                moneda: data.moneda,
            ...
            console.log("✅ Pago en efectivo registrado:", paymentResult);
          } catch (paymentError) {
            console.error(
              "❌ Error registrando pago en efectivo:",
              paymentError
            );
            // No lanzar el error para no interrumpir el flujo, solo logearlo
          }
        } else {
          // Solo generar PDF si NO es pago en efectivo
          // (Para pagos con Mercado Pago, el PDF se generará después del pago exitoso)
          console.log("📄 Generando PDF para pago no efectivo...");
          generatePDF(data);
        }
      ...
      <div className="w-full max-w-4xl mx-auto p-4">
        <div className="text-center">
          ...
          <p className="text-slate-400 mb-4">
            {wasCashPayment 
              ? "El pago en efectivo ha sido registrado. El PDF del contrato estará disponible en el historial de pagos."
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="bg-slate-800/30 backdrop-blur-sm border border-slate-700/50 rounded-xl p-4 shadow-xl">
          ...
          <div className="mb-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              {/* Mostrar selección de moneda solo si es Mercado Pago */}
              {(watchedValues.formaPago === "mercadopago" ||
                watchedValues.formaPago === "efectivo") && (
                <FormSelect
              ...
              <div className="space-y-1.5">
                <label className="block text-sm font-semibold text-slate-200">
                  Precio Total
                  {watchedValues.moneda === "ARS" && ratesLoading && (
                ...
                <div className="px-3 py-2 bg-slate-800/50 border border-slate-600/50 rounded-lg text-slate-100 text-sm">
                  {formatPrice(precioCalculado, watchedValues.moneda || "USD")}
                  {watchedValues.moneda === "ARS" && (
                    <div className="text-xs text-slate-400 mt-1">
                      Tasa: 1 USD = ${rates.ARS?.toFixed(2) || "..."} ARS
                      <br />
                      <span className="text-yellow-400">
                        ⚠️ Incluye recargo del 15% para pagos en pesos
                        argentinos
                      </span>
                    </div>
                  )}
          ...
          <div className="flex items-start space-x-2">
            ...
            <div>
              ...
              <ul className="text-xs text-slate-400 space-y-0.5">
                ...
                <li>
                  • Los pagos en pesos argentinos incluyen un recargo del 15%
                </li>
              </ul>
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-end ">
          ...
          <Button
            type="submit"
            variant="primary"
            loading={isSubmitting || mercadopagoLoading}
            disabled={!isFormValid || isSubmitting || mercadopagoLoading}
            className="w-full sm:w-auto"
          >
            {watchedValues.formaPago === "mercadopago"
              ? "Registrar y Generar Link de Pago"
      ...
      {/* Modal de Pago */}
      {showPaymentModal && createdRestaurant && (
        <PaymentModal
