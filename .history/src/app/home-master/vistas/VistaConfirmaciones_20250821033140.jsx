...
export default function VistaConfirmaciones() {
  ...
  const handleDownloadContract = async (pago) => {
    try {
      ...
      // Si no se encuentra el restaurante, crear un objeto con los datos del pago
      if (!restaurant) {
        restaurant = {
          id: pago.restaurantId,
          nombre: pago.restaurantName || "Restaurante",
          propietario: pago.owner || pago.propietario || "Propietario",
          direccion: pago.address || pago.direccion || "Dirección no especificada",
          telefono: pago.phone || pago.telefono || "Teléfono no especificado",
          email: pago.email || "Email no especificado",
          codigoActivacion: pago.codigoActivacion || pago.activationCode || "N/A",
          password: pago.password || pago.contraseña || "N/A",
          periodicidad: pago.periodicidad || getPaymentPeriodicity(pago),
        };
      }

      // Agregar datos faltantes al restaurante
      restaurant.codigoActivacion = restaurant.codigoActivacion || pago.codigoActivacion || pago.activationCode || "N/A";
      restaurant.password = restaurant.password || pago.password || pago.contraseña || "N/A";
      restaurant.periodicidad = restaurant.periodicidad || pago.periodicidad || getPaymentPeriodicity(pago);
