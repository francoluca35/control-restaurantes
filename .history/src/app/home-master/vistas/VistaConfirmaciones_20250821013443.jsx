...
export default function VistaConfirmaciones() {
  ...
  const getPaymentType = (pago) => {
    // Si tiene paymentMethod y es diferente de "cash" o "efectivo", es virtual
    if (
      pago.paymentMethod &&
      !pago.paymentMethod.toLowerCase().includes("cash") &&
      !pago.paymentMethod.toLowerCase().includes("efectivo")
    ) {
      return "virtual";
    }
    return "cash";
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredPagos.map((pago) => (
                    <tr key={pago.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <span className="text-lg mr-2">{getPaymentTypeIcon(pago)}</span>
                          <div>
                            <div className={`text-sm font-medium ${getPaymentTypeColor(pago)}`}>
                              {formatAmount(pago.amount)}
                            </div>
                            <div className="text-sm text-gray-500">
                              {pago.paymentMethod || (getPaymentType(pago) === "virtual" ? "Mercado Pago" : "Efectivo")}
                            </div>
                          </div>
                        </div>
                      </td>
