...
export default function Sidebar({ onChangeVista, onSidebarToggle }) {
  ...
  // Effect para cerrar el sidebar al mover el mouse fuera
  useEffect(() => {
    let timeoutId;
  ...
  return (
    <div
      ...
      className={`fixed left-0 top-0 h-screen bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 border-r border-gray-700/50 transition-all duration-300 ease-in-out ${
        abierto ? "w-64" : "w-20"
      } flex flex-col justify-between shadow-2xl z-50`}
    >
      {/* Header */}
      <div className="p-4">
                 {/* Logo/Brand */}
         ...
         <button
           ...
           className={`absolute top-4 right-2 w-8 h-8 bg-gray-700/50 hover:bg-gray-600/50 rounded-lg flex items-center justify-center text-gray-300 hover:text-white transition-all duration-200 ${
             abierto ? "right-2" : "right-1"
           }`}
           title={abierto ? "Contraer menú" : "Expandir menú"}
         >
           {abierto ? <FaChevronLeft size={14} /> : <FaChevronRight size={14} />}
         </button>
