  ...
  const downloadContract = useCallback(
    async (paymentData, restaurantData) => {
      try {
        ...
        doc.text("CONTRATO DE ACTIVACIÓN", 105, 25, { align: "center" });
        
        // Subtítulo
        doc.setFontSize(16);
        doc.setTextColor(100, 100, 100);
        doc.text("Sistema de Control de Restaurantes", 105, 35, { align: "center" });
        ...
        doc.text("DATOS DEL RESTAURANTE", 20, 60);
        
        doc.setFontSize(12);
        doc.setTextColor(100, 100, 100);
        doc.text(`Nombre: ${restaurantData.nombre || restaurantData.name || "N/A"}`, 20, 75);
        doc.text(`Propietario: ${restaurantData.propietario || "N/A"}`, 20, 85);
        doc.text(`Email: ${restaurantData.email || "N/A"}`, 20, 95);
        doc.text(`Teléfono: ${restaurantData.telefono || "N/A"}`, 20, 105);
        doc.text(`Dirección: ${restaurantData.direccion || "N/A"}`, 20, 115);
        doc.text(`Código de Activación: ${restaurantData.codigoActivacion || "N/A"}`, 20, 125);
        doc.text(`Cantidad de Usuarios: ${restaurantData.cantUsuarios || "1"}`, 20, 135);
        ...
        doc.setFontSize(12);
        doc.setTextColor(100, 100, 100);
        doc.text(`Tipo de Servicio: ${restaurantData.tipoServicio || "Sin Finanzas"}`, 20, 170);
        doc.text(`Forma de Pago: ${restaurantData.formaPago || "N/A"}`, 20, 180);
        doc.text(`Periodicidad: ${restaurantData.periodicidad || "Mensual"}`, 20, 190);
        ...
        doc.text(`Método de Pago: ${paymentData.paymentMethod || "N/A"}`, 20, 245);
        doc.text(`Estado: ${paymentData.status === "approved" ? "Aprobado" : "Pendiente"}`, 20, 255);
        doc.text(`ID de Pago: ${paymentData.paymentId || paymentData.id || "N/A"}`, 20, 265);
        ...
        doc.text(`Fecha de Pago: ${paymentDate.toLocaleDateString("es-AR")}`, 20, 275);
        ...
        doc.setTextColor(100, 100, 100);
        const terms = [
          "1. El servicio tiene una duración de 30 días desde la fecha de activación.",
          "2. El restaurante tendrá acceso completo al sistema de control.",
          "3. Se incluye soporte técnico durante el período contratado.",
          "4. Los datos del restaurante serán tratados con confidencialidad.",
          "5. El pago debe realizarse antes de la activación del servicio.",
          "6. Cualquier modificación debe ser notificada por escrito."
        ];
        
        let yPosition = 310;
        terms.forEach((term, index) => {
          if (yPosition > 250) { // Si hay espacio en la página
        ...
        doc.setFontSize(12);
        doc.setTextColor(0, 0, 0);
        doc.text("Firma del Cliente:", 20, yPosition + 20);
        doc.line(20, yPosition + 25, 80, yPosition + 25);
        
        doc.text("Firma del Representante:", 120, yPosition + 20);
        doc.line(120, yPosition + 25, 180, yPosition + 25);
        
        // Fecha de generación
        doc.setFontSize(10);
        doc.setTextColor(150, 150, 150);
        doc.text(`Generado el: ${new Date().toLocaleDateString("es-AR")}`, 20, 280);
        
        // Generar nombre del archivo
        const restaurantName = (restaurantData.nombre || restaurantData.name || "restaurante")
