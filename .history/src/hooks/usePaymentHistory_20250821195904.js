  ...
  const downloadContract = useCallback(
    async (paymentData, restaurantData) => {
      try {
        ...
        // Crear formas curvas reales usando paths
        // Curva superior izquierda
        doc.setFillColor(...violetColor);
        doc.setDrawColor(...blackColor);
        doc.setLineWidth(0.5);

        // Forma curva superior (simulada con rectángulo redondeado)
        doc.roundedRect(0, 0, 210, 40, 3, 3, "F");
        doc.roundedRect(0, 0, 210, 40, 3, 3, "D");

        // Forma curva inferior
        doc.roundedRect(0, 250, 210, 40, 3, 3, "F");
        doc.roundedRect(0, 250, 210, 40, 3, 3, "D");
        ...
        doc.setFontSize(12);
        doc.setTextColor(...violetColor);
        doc.text("Servicios de", 20, 60);
        doc.text("Fecha", 70, 60);
        doc.text("Contacto", 120, 60);
        
        doc.setFontSize(11);
        doc.setTextColor(...blackColor);
        doc.text("DeamonDD By FranComputer", 20, 70);
        doc.text(new Date().toLocaleDateString("es-AR"), 70, 70);
        doc.text("contacto@deamondd.com", 120, 70);
        
        // Líneas separadoras verticales
        doc.setDrawColor(...grayColor);
        doc.setLineWidth(0.5);
        doc.line(65, 55, 65, 75);
        doc.line(115, 55, 115, 75);
        ...
        doc.setFontSize(14);
        doc.setTextColor(...violetColor);
        doc.text("Contrato de servicios digitales", 20, 95);
        
        // Información completa del restaurante
        doc.setFontSize(12);
        doc.setTextColor(...violetColor);
        doc.text("DATOS DEL RESTAURANTE", 20, 115);
        
        doc.setFontSize(10);
        doc.setTextColor(...blackColor);
        doc.text(`Nombre del Restaurante: ${restaurantData.nombre || restaurantData.name || "N/A"}`, 20, 125);
        doc.text(`Nombre Completo: ${restaurantData.propietario || "N/A"}`, 20, 135);
        doc.text(`DNI: ${restaurantData.dni || "N/A"}`, 20, 145);
        doc.text(`Email: ${restaurantData.email || "N/A"}`, 20, 155);
        doc.text(`Teléfono: ${restaurantData.telefono || "N/A"}`, 20, 165);
        doc.text(`Dirección: ${restaurantData.direccion || "N/A"}`, 20, 175);
        doc.text(`Código de Activación: ${restaurantData.codigoActivacion || "N/A"}`, 20, 185);
        doc.text(`Cantidad de Usuarios: ${restaurantData.cantUsuarios || "1"}`, 20, 195);
        
        // Información del servicio
        doc.setFontSize(12);
        doc.setTextColor(...violetColor);
        doc.text("CONFIGURACIÓN DEL SERVICIO", 20, 215);
        
        doc.setFontSize(10);
        doc.setTextColor(...blackColor);
        doc.text(`Tipo de Servicio: ${restaurantData.tipoServicio || "Sin Finanzas"}`, 20, 225);
        doc.text(`Forma de Pago: ${restaurantData.formaPago || "N/A"}`, 20, 235);
        doc.text(`Periodicidad: ${restaurantData.periodicidad || "Mensual"}`, 20, 245);
        doc.text(`Moneda: ${restaurantData.moneda || "USD"}`, 20, 255);
