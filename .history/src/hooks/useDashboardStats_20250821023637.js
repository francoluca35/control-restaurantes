    ...
    try {
      ...
      for (const doc of paymentsSnapshot.docs) {
        ...
        if (paymentData.status === "approved") {
          ...
          // Si contiene "CASH" es efectivo, sino es virtual (Mercado Pago)
          const isCashPayment =
            paymentData.paymentId &&
            typeof paymentData.paymentId === "string" &&
            paymentData.paymentId.toUpperCase().includes("CASH");

          // Determinar si es anual o mensual basándose en la periodicidad o el ID
          ...
          // Determinar la moneda del pago
          let isARS = false;
          let isUSD = false;

          if (isCashPayment) {
            // Para pagos en efectivo, usar el ID del pago
            isARS = paymentData.paymentId && 
                   typeof paymentData.paymentId === "string" && 
                   paymentData.paymentId.toUpperCase().includes("ARS");
            isUSD = paymentData.paymentId && 
                   typeof paymentData.paymentId === "string" && 
                   paymentData.paymentId.toUpperCase().includes("USD");
          } else {
            // Para pagos de Mercado Pago, buscar el restaurante para obtener es_usd
            try {
              const restaurantDoc = await getDoc(doc(db, "restaurantes", paymentData.restaurantId));
              if (restaurantDoc.exists()) {
                const restaurantData = restaurantDoc.data();
                isUSD = restaurantData.es_usd === true;
                isARS = restaurantData.es_usd === false;
              } else {
                // Fallback: usar campos de la transacción
                isARS = paymentData.currency === "ARS" || paymentData.moneda === "ARS";
                isUSD = paymentData.currency === "USD" || paymentData.moneda === "USD";
              }
            } catch (error) {
          ...
          // Determinar si es USD usando el nuevo campo
          const isUSD =
            paymentData.currency === "USD" ||
            paymentData.moneda === "USD" ||
            paymentData.es_usd === true ||
            (paymentData.paymentId &&
              typeof paymentData.paymentId === "string" &&
              paymentData.paymentId.toUpperCase().includes("USD"));
          ...
          if (isAnnualPayment) {
            if (isARS) {
              ...
              // Por defecto ARS si no se puede determinar
              ingresosAnualesARS += amount;
            }
          } else {
            if (isARS) {
