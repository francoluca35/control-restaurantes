  ...
  const loadDashboardStats = useCallback(async () => {
    try {
      ...
      paymentsSnapshot.docs.forEach((doc) => {
        ...
        if (paymentData.status === "approved" || paymentData.status === "pagado") {
          pagosTotales++;
          ...
          // Determinar la moneda del pago usando el campo es_usd
          let isARS = false;
          let isUSD = false;

          if (paymentData.es_usd !== undefined) {
            // Usar el campo es_usd si está disponible
            isUSD = paymentData.es_usd === true;
            isARS = paymentData.es_usd === false;
          } else if (isCashPayment) {
            // Para pagos en efectivo, usar el ID del pago como fallback
            isARS =
              paymentData.paymentId &&
              typeof paymentData.paymentId === "string" &&
            ...
            // Para pagos de Mercado Pago, usar campos de la transacción como fallback
            isARS =
