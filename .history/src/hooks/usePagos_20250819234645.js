...
export const usePagos = () => {
  ...
  const getPaymentStats = useCallback(() => {
    ...
    const ingresosMensuales = restaurants.reduce((total, restaurant) => {
      // Si el restaurante tiene ingresos mensuales registrados, usarlos
      if (restaurant.ingresosMensuales) {
        return total + restaurant.ingresosMensuales;
      }
      
      // Si no, calcular basado en el precio y periodicidad
      if (restaurant.periodicidad === "mensual") {
        return total + (restaurant.precio || 0);
      } else {
        return total + (restaurant.precio || 0) / 12;
      }
    }, 0);
