  ...
  const downloadContract = useCallback(
    async (paymentData, restaurantData) => {
      try {
        ...
        const contractText = [
          `El Proveedor (DeamonDD By FranComputer) se compromete a proporcionar servicios de sistema de control de restaurantes al Cliente (${restaurantData.nombre || restaurantData.name || "Restaurante"}) según los términos establecidos en este contrato.`,
          
          `El Cliente se compromete a realizar el pago de $${paymentData.amount || "0"} por ${restaurantData.periodicidad || "mensual"} de servicios, con ${restaurantData.tipoServicio || "Sin Finanzas"} incluidos. El servicio incluye acceso completo al sistema, soporte técnico y actualizaciones durante el período contratado.`,
        ...
        doc.text("(11) 1234-5678 // contacto@deamondd.com // Av. Principal 123, Buenos Aires", 20, yPosition + 10);
