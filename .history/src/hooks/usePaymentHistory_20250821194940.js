  ...
  const downloadContract = useCallback(
    async (paymentData, restaurantData) => {
      try {
        ...
        doc.text("CONTRATO DE ACTIVACIÓN DE RESTAURANTE", 105, 20, { align: "center" });
        ...
        doc.text(`Nombre: ${restaurantData.nombre || restaurantData.name || "N/A"}`, 20, 55);
        doc.text(`Propietario: ${restaurantData.propietario || "N/A"}`, 20, 65);
        doc.text(`Email: ${restaurantData.email || "N/A"}`, 20, 75);
        doc.text(`Teléfono: ${restaurantData.telefono || "N/A"}`, 20, 85);
        doc.text(`Dirección: ${restaurantData.direccion || "N/A"}`, 20, 95);
        doc.text(`Código de Activación: ${restaurantData.codigoActivacion || "N/A"}`, 20, 105);
        doc.text(`Cantidad de Usuarios: ${restaurantData.cantUsuarios || "1"}`, 20, 115);
        
        // Configuración del servicio
        doc.setFontSize(14);
        doc.setTextColor(0, 0, 0);
        doc.text("CONFIGURACIÓN DEL SERVICIO", 20, 135);
        
        doc.setFontSize(12);
        doc.setTextColor(100, 100, 100);
        doc.text(`Tipo de Servicio: ${restaurantData.tipoServicio || "Sin Finanzas"}`, 20, 150);
        doc.text(`Forma de Pago: ${restaurantData.formaPago || "N/A"}`, 20, 160);
        doc.text(`Periodicidad: ${restaurantData.periodicidad || "Mensual"}`, 20, 170);
        doc.text(`Moneda: ${restaurantData.moneda || "USD"}`, 20, 180);
        
        // Información del pago
        doc.setFontSize(14);
        doc.setTextColor(0, 0, 0);
        doc.text("INFORMACIÓN DEL PAGO", 20, 200);
        
        doc.setFontSize(12);
        doc.setTextColor(100, 100, 100);
        doc.text(`Monto: $${paymentData.amount || "0"}`, 20, 215);
        doc.text(`Método de Pago: ${paymentData.paymentMethod || "N/A"}`, 20, 225);
        doc.text(`Estado: ${paymentData.status === "approved" ? "Aprobado" : "Pendiente"}`, 20, 235);
        doc.text(`ID de Pago: ${paymentData.paymentId || paymentData.id || "N/A"}`, 20, 245);
        ...
        const restaurantName = (restaurantData.nombre || restaurantData.name || "restaurante").replace(/[^a-zA-Z0-9]/g, "_");
