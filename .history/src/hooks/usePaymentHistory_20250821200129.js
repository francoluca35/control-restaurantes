  ...
  const downloadContract = useCallback(
    async (paymentData, restaurantData) => {
      try {
        ...
        doc.setFontSize(10);
        doc.setTextColor(...blackColor);
        doc.text(`Nombre del Restaurante: ${restaurantData.nombre || restaurantData.name || "N/A"}`, 20, 120);
        doc.text(`Propietario: ${restaurantData.propietario || "N/A"}`, 20, 130);
        ...
        doc.text(`Código de Activación: ${restaurantData.codigoActivacion || "N/A"}`, 20, 180);
        doc.text(`Cantidad de Usuarios: ${restaurantData.cantUsuarios || "1"}`, 20, 190);
        ...
        doc.text(`Periodicidad: ${restaurantData.periodicidad || "Mensual"}`, 20, 235);
        doc.text(`Cantidad de Usuarios: ${restaurantData.cantUsuarios || "1"}`, 20, 245);
