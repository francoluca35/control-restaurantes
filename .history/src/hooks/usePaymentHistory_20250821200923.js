  ...
  const downloadContract = useCallback(
    async (paymentData, restaurantData) => {
      try {
        ...
        // Subtítulo
        doc.setFontSize(12);
        doc.text("Sistema de Comandas Multiples", 105, 25, {
          align: "center",
        });
        ...
        doc.text("INFORMACIÓN DEL CONTACTO", 20, 55);

        doc.setFontSize(10);
        doc.setTextColor(...blackColor);
        doc.text(
          `Fecha de contrato: ${new Date().toLocaleDateString("es-AR")}`,
          20,
          65
        );
        ...
        doc.text(
          `Nombre Completo: ${restaurantData.propietario || "N/A"}`,
          20,
          130
        );
        ...
        doc.setFontSize(16);
        doc.setTextColor(...violetColor);
        doc.text("TERMINOS & CONDICIONES", 20, yPosition + 20);

        doc.setFontSize(10);
        doc.setTextColor(...blackColor);
        const terms = [
          "1. DURACIÓN: El servicio tiene una duración de 30 días desde la fecha de activación.",
          "2. ACCESO: El cliente tendrá acceso completo al sistema de control de restaurantes.",
          "3. SOPORTE: Se incluye soporte técnico durante el período contratado.",
          "4. CONFIDENCIALIDAD: Los datos del cliente serán tratados con total confidencialidad.",
        ...
        doc.setFontSize(14);
        doc.setTextColor(...violetColor);
        doc.text("FIRMAS", 20, termsYPosition + 30);

        // Nombre del representante
        doc.setFontSize(12);
        doc.setTextColor(...blackColor);
        doc.text("Franco Luca Parera", 105, termsYPosition + 45, { align: "center" });
        
        // Logo en el centro
        doc.setFontSize(16);
        doc.setTextColor(...violetColor);
        doc.text("LOGO", 105, termsYPosition + 60, { align: "center" });
        
        // Líneas de firma
        doc.setDrawColor(...blackColor);
        doc.setLineWidth(1);
        doc.line(20, termsYPosition + 75, 90, termsYPosition + 75);
        doc.line(120, termsYPosition + 75, 190, termsYPosition + 75);

        // Etiquetas de las firmas
        doc.setFontSize(10);
        doc.setTextColor(...blackColor);
        doc.text("Firma del Cliente", 55, termsYPosition + 90, { align: "center" });
        doc.text("Firma Empresarial", 155, termsYPosition + 90, { align: "center" });
