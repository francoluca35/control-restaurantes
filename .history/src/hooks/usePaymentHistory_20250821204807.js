  ...
  const downloadContract = useCallback(
    async (paymentData, restaurantData) => {
      try {
        ...
        currentY += 8;
        doc.text("El Proveedor se compromete a brindarle un servicio de calidad al Cliente, incluyendo la planificación, ejecución y supervisión de estrategias de contenido.", 25, currentY);
        currentY += 8;
        doc.text("El Cliente se compromete a proporcionar toda la información y recursos necesarios para la creación del contenido y a aprobar o solicitar modificaciones de las publicaciones en un plazo razonable.", 25, currentY);
        currentY += 8;
        doc.text("El Cliente abonará al Proveedor una tarifa mensual de $1000, que se pagará al inicio de cada mes de servicio. Este contrato tendrá una duración de 30 días, renovable automáticamente por periodos iguales salvo notificación escrita por cualquiera de las partes con 10 días de antelación.", 25, currentY);
        currentY += 8;
        doc.text("Ambas partes acuerdan trabajar de manera colaborativa y comunicarse de manera efectiva para el éxito de las estrategias digitales implementadas.", 25, currentY);
