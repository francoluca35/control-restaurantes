  ...
  const downloadContract = useCallback(
    async (paymentData, restaurantData) => {
      try {
        ...
        doc.setFontSize(12);
        doc.setTextColor(...blackColor);
        doc.text(restaurantData.nombre || restaurantData.name || "RESTAURANTE", 55, termsYPosition + 45, { align: "center" });
        doc.text("DEAMONDD BY FRANCOMPUTER", 155, termsYPosition + 45, { align: "center" });
