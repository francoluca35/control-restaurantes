  ...
  const downloadContract = useCallback(
    async (paymentData, restaurantData) => {
      try {
        ...
        doc.setFontSize(18);
        doc.setFont("helvetica", "bold");
        doc.text("CONTRATO DE SERVICIOS TECNOLÓGICOS", 105, 15, { align: "center" });
        
        doc.setFontSize(12);
        doc.setFont("helvetica", "normal");
        doc.text("Sistema de Gestión para Restaurantes", 105, 25, { align: "center" });
        ...
        const contractNumber = `CW-${new Date().getFullYear()}-${String(restaurantData.codigoActivacion || Date.now()).padStart(6, '0')}`;
        ...
        currentY += 15;
        doc.text(`Vigencia: 30 días desde la fecha de activación`, 20, currentY);
        ...
        currentY += 8;
        doc.text("Dirección: Av. Principal 123, Buenos Aires, Argentina", 25, currentY);
        currentY += 8;
        doc.text("Email: contacto@deamondd.com", 25, currentY);
        currentY += 8;
        doc.text("Teléfono: (11) 1234-5678", 25, currentY);
        currentY += 8;
        doc.text("CUIT: 20-12345678-9", 25, currentY);
        ...
        doc.setFont("helvetica", "normal");
        doc.setTextColor(...textColor);
        doc.text(`Razón Social: ${restaurantData.nombre || restaurantData.name || "N/A"}`, 25, currentY);
        currentY += 8;
        doc.text(`Representante: ${restaurantData.propietario || restaurantData.nombreCompleto || "N/A"}`, 25, currentY);
        currentY += 8;
        doc.text(`DNI: ${restaurantData.dni || "N/A"}`, 25, currentY);
        currentY += 8;
        doc.text(`Dirección: ${restaurantData.direccion || restaurantData.address || "N/A"}`, 25, currentY);
        currentY += 8;
        doc.text(`Email: ${restaurantData.email || "N/A"}`, 25, currentY);
        currentY += 8;
        doc.text(`Teléfono: ${restaurantData.telefono || restaurantData.phone || "N/A"}`, 25, currentY);
        ...
        doc.text(`Código de Activación: ${restaurantData.codigoActivacion || "N/A"}`, 20, currentY);
        currentY += 8;
        doc.text(`Cantidad de Usuarios: ${restaurantData.cantUsuarios || "1"}`, 20, currentY);
        currentY += 8;
        doc.text(`Tipo de Servicio: ${restaurantData.tipoServicio || "Básico"}`, 20, currentY);
        currentY += 8;
        doc.text(`Módulo de Finanzas: ${restaurantData.finanzas ? "Incluido" : "No incluido"}`, 20, currentY);
        currentY += 8;
        doc.text(`Periodicidad: ${restaurantData.periodicidad || "Mensual"}`, 20, currentY);
        ...
        doc.text(`Monto del Servicio: $${paymentData.amount || paymentData.transaction_amount || "0"} ${paymentData.currency || paymentData.currency_id || "ARS"}`, 20, currentY);
        currentY += 8;
        doc.text(`Método de Pago: ${paymentData.payment_method?.type || paymentData.paymentMethod || "MercadoPago"}`, 20, currentY);
        currentY += 8;
        doc.text(`ID de Transacción: ${paymentData.id || paymentData.transactionId || "N/A"}`, 20, currentY);
        currentY += 8;
        doc.text(`Estado del Pago: ${paymentData.status === "approved" ? "Aprobado" : "Pendiente"}`, 20, currentY);
        currentY += 8;
        doc.text(`Fecha de Pago: ${paymentDate.toLocaleDateString("es-AR")}`, 20, currentY);
        currentY += 8;
        ...
        doc.setFont("helvetica", "normal");
        doc.setTextColor(...textColor);
        doc.text(`${restaurantData.propietario || restaurantData.nombreCompleto || "Cliente"}`, 55, currentY + 20, { align: "center" });
        doc.text(`DNI: ${restaurantData.dni || "N/A"}`, 55, currentY + 28, { align: "center" });
        ...
        doc.setFont("helvetica", "bold");
        doc.setTextColor(...secondaryColor);
        doc.text("Firma del Prestador", 155, currentY + 10, { align: "center" });
        
        doc.setFontSize(10);
        doc.setFont("helvetica", "normal");
        doc.setTextColor(...textColor);
        doc.text("Franco Luca Parera", 155, currentY + 20, { align: "center" });
        doc.text("DeamonDD By FranComputer", 155, currentY + 28, { align: "center" });
        ...
        doc.setFontSize(8);
        doc.setTextColor(...secondaryColor);
        doc.text("Este documento es generado automáticamente y tiene validez legal", 105, footerY + 8, { align: "center" });
        doc.text("Para consultas: contacto@deamondd.com | Tel: (11) 1234-5678", 105, footerY + 15, { align: "center" });
        doc.text(`Generado el: ${new Date().toLocaleDateString("es-AR")} a las ${new Date().toLocaleTimeString("es-AR")}`, 105, footerY + 22, { align: "center" });
        ...
        return { success: true, message: "Contrato empresarial descargado exitosamente" };
      } catch (err) {
