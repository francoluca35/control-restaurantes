  ...
  const downloadContract = useCallback(
    async (paymentData, restaurantData) => {
      try {
        ...
        doc.setFont("helvetica", "bold");
        doc.setTextColor(...primaryColor);
        doc.text("CONTRATO DE SERVICIOS DIGITALES", 20, currentY);
        currentY += 8;
        doc.text("El Proveedor se compromete a gestionar la cuenta digital del Cliente, incluyendo la planificación, ejecución y supervisión de estrategias de contenido.

El Cliente se compromete a proporcionar toda la información y recursos necesarios para la creación del contenido y a aprobar o solicitar modificaciones de las publicaciones en un plazo razonable. 

El Cliente abonará al Proveedor una tarifa mensual de [monto acordado], que se pagará al inicio de cada mes de servicio. Este contrato tendrá una duración de [duración acordada], renovable automáticamente por periodos iguales salvo notificación escrita por cualquiera de las partes con [número de días] días de antelación. Ambas partes acuerdan trabajar de manera colaborativa y comunicarse de manera efectiva para el éxito de las estrategias digitales implementadas.

(55) 1234-5678 // hola@sitioincreible.com // Calle Cualquiera 123, Cualquier Lugar
