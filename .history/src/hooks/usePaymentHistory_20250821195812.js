  ...
  const downloadContract = useCallback(
    async (paymentData, restaurantData) => {
      try {
        ...
        // Definir colores del diseño (violeta en lugar de rojo)
        const violetColor = [139, 92, 246]; // Violet-500
        const blackColor = [0, 0, 0];
        const grayColor = [156, 163, 175]; // Gray-400
        
        // Crear formas onduladas decorativas
        // Forma ondulada superior
        doc.setFillColor(...violetColor);
        doc.setDrawColor(...blackColor);
        doc.setLineWidth(0.5);
        
        // Crear forma ondulada superior usando múltiples rectángulos redondeados
        doc.roundedRect(0, 0, 210, 35, 5, 5, 'F');
        doc.roundedRect(0, 0, 210, 35, 5, 5, 'D');
        
        // Forma ondulada inferior
        doc.roundedRect(0, 260, 210, 35, 5, 5, 'F');
        doc.roundedRect(0, 260, 210, 35, 5, 5, 'D');
        ...
        doc.text("CONTRATO DIGITAL", 105, 25, { align: "center" });
        ...
        doc.setFontSize(12);
        doc.setTextColor(...violetColor);
        doc.text("Servicios de", 20, 55);
        doc.text("Fecha", 70, 55);
        doc.text("Contacto", 120, 55);
        
        doc.setFontSize(11);
        doc.setTextColor(...blackColor);
        doc.text("DeamonDD By FranComputer", 20, 65);
        doc.text(new Date().toLocaleDateString("es-AR"), 70, 65);
        doc.text("contacto@deamondd.com", 120, 65);
        
        // Líneas separadoras verticales
        doc.setDrawColor(...grayColor);
        doc.setLineWidth(0.5);
        doc.line(65, 50, 65, 70);
        doc.line(115, 50, 115, 70);
        ...
        doc.setFontSize(14);
        doc.setTextColor(...violetColor);
        doc.text("Contrato de servicios digitales", 20, 90);
