...
export const restaurantSchema = z
  .object({
    ...
    dni: z
      ...
      .max(10, "El DNI es demasiado largo")
    ...
    formaPago: z.enum(["efectivo", "mercadopago"], {
      errorMap: () => ({
  ...
  .refine(
    (data) => {
      // Si la forma de pago es efectivo o mercadopago, la moneda es requerida
      if (
        (data.formaPago === "efectivo" || data.formaPago === "mercadopago") &&
        !data.moneda
      ) {
        return false;
      }
    ...
    {
      message:
        "Debe seleccionar una moneda cuando el pago es con Mercado Pago o Efectivo",
      path: ["moneda"], // Especifica que el error va al campo moneda
    }
  );
...
export const paymentSchema = z.object({
  ...
  method: z.enum(["efectivo", "mercadopago"], {
    errorMap: () => ({
      message: "El método de pago seleccionado no es válido",
    }),
  }),
