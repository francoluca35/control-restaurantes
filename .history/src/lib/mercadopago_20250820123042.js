...
export const createPaymentPreference = async (paymentData) => {
  ...
  // Si no está configurado, crear una preferencia de prueba
  if (!isConfigured) {
    console.log("🧪 Creando preferencia de pago de prueba...");

    // Simular una respuesta de MercadoPago para pruebas
    const testPreference = {
      id: `test_pref_${Date.now()}`,
      init_point:
        "https://www.mercadopago.com.ar/checkout/v1/redirect?pref_id=TEST_PREFERENCE_ID",
      sandbox_init_point:
        "https://sandbox.mercadopago.com.ar/checkout/v1/redirect?pref_id=TEST_PREFERENCE_ID",
    ...
    console.log("✅ Preferencia de prueba creada:", testPreference.id);
