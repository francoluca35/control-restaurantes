    ...
    try {
      ...
      if (data.success) {
        ...
        const envCheck = {
          ...
          mercadopago: {
            // Solo verificar la clave pública en el frontend
            publicKey: data.mercadopago.status.publicKey === "✅ Configurada",
            // El access token se verifica en el backend, no en el frontend
            accessToken: "backend-only", // No verificamos esto en el frontend
          },
        };
        ...
        // Verificar MercadoPago - Solo verificar que la API responda correctamente
        try {
          ...
          if (mpResponse.status === 500) {
            ...
            if (errorData.error?.includes("Mercado Pago no está configurado")) {
              ...
              // Si el error no es de configuración, entonces MercadoPago está funcionando
