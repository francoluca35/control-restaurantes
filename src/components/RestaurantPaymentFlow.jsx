    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center space-x-4">
          <div
            className={`flex items-center ${
              currentStep === "generate"
                ? "text-blue-600"
                : currentStep === "monitor" || currentStep === "contract"
                ? "text-green-600"
                : "text-gray-400"
            }`}
          >
          ...
          <div
            className={`flex-1 h-1 ${
              currentStep === "contract" ? "bg-green-600" : "bg-gray-200"
            }`}
          ></div>

          <div
            className={`flex items-center ${
              currentStep === "contract" ? "text-blue-600" : "text-gray-400"
            }`}
          >
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <span className="ml-2 font-medium">
                {restaurantData.propietario || restaurantData.owner}
              </span>
            </div>
            <div>
              <span className="text-gray-600">Dirección:</span>
              <span className="ml-2 font-medium">
                {restaurantData.direccion || restaurantData.address}
              </span>
            </div>
            <div>
              <span className="text-gray-600">Teléfono:</span>
              <span className="ml-2 font-medium">
                {restaurantData.telefono || restaurantData.phone}
              </span>
            </div>
            <div>
      ...
      {paymentInfo && (
        <div className="bg-blue-50 p-4 rounded-lg">
          <h4 className="font-medium text-blue-900 mb-2">Estado del Proceso</h4>
          <div className="text-sm text-blue-800">
            <p>
              <strong>Link generado:</strong>{" "}
              {paymentInfo.paymentUrl ? "✅ Sí" : "❌ No"}
            </p>
            <p>
              <strong>Preference ID:</strong> {paymentInfo.preferenceId}
            </p>
            <p>
              <strong>Restaurant ID:</strong> {paymentInfo.restaurantId}
            </p>
            {paymentStatus && (
              <p>
                <strong>Estado del pago:</strong> {paymentStatus.status}
              </p>
            )}
          </div>
        </div>
